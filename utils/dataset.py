...
#     def __init__(self, imgs_dir, masks_dir, mask_suffix='_segmentation', transform=None):
...
class BasicDataset(Dataset):
    def __init__(self, imgs_dir, masks_dir, mask_suffix='manual1'):
        self.imgs_dir = imgs_dir
        self.masks_dir = masks_dir
        self.mask_suffix = mask_suffix
    ...
    def __getitem__(self, i):
        ...
        # )
        mask_file = glob(
            osp.join(self.masks_dir, idm + self.mask_suffix + '.*'))
        img_file = glob(osp.join(self.imgs_dir, idx + '.*'))
